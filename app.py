        ...
        if article_sections:
            ...
            for idx, sec in enumerate(article_sections):
                ...
                if sec["mode"] == "manual" and sec["heading"]:
                    ...
                    mode_text = "SEOを意識した見出しにしてください"  # AI生成の場合の表示
...
"""
        ...
        prompt = f"""
##状況 
あなたは高度なコンテンツクリエイター兼SEOに非常に詳しいnote編集者です。 

##指示 
読者に価値あるnote記事を段落ごとに順番に生成してください。
...
【段落生成条件】
0. 文字総量指定は{constraint_length}ですが、1000文字ほど多めに見積もってください。
...
9. 各段落を生成したら、その段落だけを出力してください。次の段落は別で生成してください。
10. 段落生成後、コピーして順番に貼り付けるだけで記事完成できる設計にしてください。
11. 必ず記事の個性が出る様にしてください。
